...
bool JfrStackTrace::record(JavaThread* current_thread, const frame& frame, int skip, int64_t stack_frame_id) {
  assert(current_thread == JavaThread::current(), "must be current thread");
  ResetNoHandleMark rnhm;
  HandleMark hm(current_thread);
  return record_inner(current_thread, frame, skip, stack_frame_id);
}

bool JfrStackTrace::record_inner(JavaThread* jt, const frame& frame, int skip, int64_t stack_frame_id) {
  ...
  const JfrStackFilter* stack_filter = JfrStackFilterRegistry::lookup(stack_frame_id);
  _hash = 1;
  while (!vfs.at_end()) {
