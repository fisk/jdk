...
bool JfrStackTrace::record(JavaThread* current_thread, const frame& frame, int skip) {
  assert(current_thread == JavaThread::current(), "must be current thread");
  ResetNoHandleMark rnhm;
  HandleMark hm(current_thread);
  return record_inner(current_thread, frame, skip);
}

bool JfrStackTrace::record_inner(JavaThread* jt, const frame& frame, int skip) {
