...
#undef C2_STUB_FIELD_DECLARE
  ...
  static void new_array_impl(Klass* array_klass, int len, oopDesc* init_val, bool local, JavaThread* current);
  ...
  // Allocate storage for a objArray or typeArray
  static void new_local_array_C(Klass* array_klass, int len, oopDesc* init_val, JavaThread* current);
