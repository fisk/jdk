...
// array allocation
void OptoRuntime::new_array_impl(Klass* array_type, int len, oopDesc* init_val, bool local, JavaThread* current) {
  JRT_BLOCK;
#ifndef PRODUCT
  ...
  if (array_type->is_typeArray_klass()) {
    ...
    result = ObjArrayKlass::cast(array_type)->allocate_instance(len, local, THREAD);
...
JRT_BLOCK_ENTRY(void, OptoRuntime::new_array_C(Klass* array_type, int len, oopDesc* init_val, JavaThread* current))
  new_array_impl(array_type, len, init_val, false /* local */, current);
JRT_END

JRT_BLOCK_ENTRY(void, OptoRuntime::new_local_array_C(Klass* array_type, int len, oopDesc* init_val, JavaThread* current))
  new_array_impl(array_type, len, init_val, true /* local */, current);
JRT_END
