...
void Parse::add_safepoint() {
  ...
  ByteSize poll_load_offset = VM_Version::supports_cond_branch_loop_polls() ? JavaThread::polling_word_offset() : JavaThread::polling_page_offset();
  Node *polling_page_load_addr = _gvn.transform(basic_plus_adr(top(), thread, in_bytes(poll_load_offset)));
