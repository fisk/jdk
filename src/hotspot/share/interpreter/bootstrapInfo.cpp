  ...
  for (SignatureStream ss(sig); !ss.is_done(); ss.next()) {
    if (ss.is_reference()) {
      ...
      if (check_return_type && ss.at_return_type()) {
        ...
        if (InstanceKlass::cast(k)->runtime_dependence_diagnosis() != nullptr) {
...
bool BootstrapInfo::is_runtime_independent() const {
  ConstantPool* cp = _pool();
  InstanceKlass* pool_holder = cp->pool_holder();
  if (!SystemDictionaryShared::is_builtin(pool_holder)) {
    return false;
  }

  int cp_index = _bss_index;
