...
public:
  ...
  ObjectMonitor* prepare_insert(oop obj, ObjectMonitor* new_monitor, int hash) {
  ...
  ObjectMonitor* get_set(oop obj, ObjectMonitor* new_monitor, int hash) {
    ...
    Table* prev = AtomicAccess::load_acquire(&_prev);
    if (prev != nullptr) {
      // Sprinkle tomb stones in previous tables to bring concurrent inserters
      // towards the latest table. We only really want to try inserting in the
      // perceivably latest table.
      ObjectMonitor* result = prev->prepare_insert(obj, new_monitor, hash);
      if (result != nullptr) {
        return result;
      }
  ...
  void rebuild() {
    ...
    for (size_t index = 0; index <= prev->_capacity_mask; index++) {
      ...
      if (monitor != tomb_stone() && monitor != removed_entry()) {
        ...
        if (obj != nullptr) {
          assert(!monitor->is_being_async_deflated(), "should already have been removed");
