...
void SafepointMechanism::update_poll_values(JavaThread* thread) {
  ...
  for (;;) {
    bool armed = global_poll() || thread->handshake_state()->has_operation() || JfrThreadSampling::has_requested_sampling(thread);
    ...
    OrderAccess::fence();
    if (!armed && (global_poll() || thread->handshake_state()->has_operation() || JfrThreadSampling::has_requested_sampling(thread))) {
...
void SafepointMechanism::process(JavaThread *thread, bool allow_suspend, bool check_async_exception) {
  ...
  do {
    if (JfrThreadSampling::has_requested_sampling(thread)) {
      JfrThreadSampling::handle_requested_sampling(thread);
    }
