...
#ifndef SHARE_UTILITIES_PRIORITYQUEUE_INLINE_HPP
#define SHARE_UTILITIES_PRIORITYQUEUE_INLINE_HPP

#include "utilities/priorityQueue.hpp"

template <typename T, MEMFLAGS flags>
inline PriorityQueue<T, flags>::PriorityQueue(IsHigherPriority is_higher_priority)
  : _array(),
    _is_higher_priority(is_higher_priority) {}

template <typename T, MEMFLAGS flags>
inline T* PriorityQueue<T, flags>::elem(int current_index) const {
...
template <typename T, MEMFLAGS flags>
inline int PriorityQueue<T, flags>::parent(int current_index) const {
  if (current_index == 0) {
...
template <typename T, MEMFLAGS flags>
inline int PriorityQueue<T, flags>::left(int current_index) const {
  return current_index * 2 + 1;
}

template <typename T, MEMFLAGS flags>
inline int PriorityQueue<T, flags>::right(int current_index) const {
  return current_index * 2 + 2;
}

template <typename T, MEMFLAGS flags>
inline void PriorityQueue<T, flags>::swap(int n1_index, int n2_index) {
...
template <typename T, MEMFLAGS flags>
inline void PriorityQueue<T, flags>::move_down(int current_index) {
  for (;;) {
...
template <typename T, MEMFLAGS flags>
inline void PriorityQueue<T, flags>::move_up(int current_index) {
  for (;;) {
...
template <typename T, MEMFLAGS flags>
void PriorityQueue<T, flags>::insert(T* element) {
...
template <typename T, MEMFLAGS flags>
T* PriorityQueue<T, flags>::first() const {
  if (_array.length() == 0) {
    return nullptr;
  }
...
template <typename T, MEMFLAGS flags>
T* PriorityQueue<T, flags>::remove_first() {
...
template <typename T, MEMFLAGS flags>
int PriorityQueue<T, flags>::length() const {
  return _array.length();
}

template <typename T, MEMFLAGS flags>
bool PriorityQueue<T, flags>::is_empty() const {
  return _array.is_empty();
}

#endif // SHARE_UTILITIES_PRIORITYQUEUE_INLINE_HPP
