...
objArrayOop ObjArrayKlass::allocate_instance(int length, bool local, TRAPS) {
  ...
  assert(is_refined_objArray_klass(), "must allocate using a refined array klass");
  size_t size;
  if (is_refArray_klass()) {
    check_array_allocation_length(length, arrayOopDesc::max_array_length(T_OBJECT), CHECK_NULL);
    size = refArrayOopDesc::object_size(length);
  } else {
    assert(is_flatArray_klass(), "unexpected array klass kind");
    FlatArrayKlass* fak = FlatArrayKlass::cast(this);
    check_array_allocation_length(length, fak->max_elements(), CHECK_NULL);
    size = flatArrayOopDesc::object_size(layout_helper(), length);
  }
  oop array = Universe::heap()->array_allocate_local(this, size, length,
