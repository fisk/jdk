...
// Support permanent objects.
int MappingArchiveHeapLoader::_permobj_segments = 0;
...
oop MappingArchiveHeapLoader::get_archived_object(int permanent_index) {
  assert(permobj_segments() > 0, "must be");
...
void MappingArchiveHeapLoader::populate_permanent_object_table() {
  int permobj_segments = MappingArchiveHeapLoader::permobj_segments();
  int first_permobj_segment = HeapShared::roots()->length() - permobj_segments;
...
void MappingArchiveHeapLoader::serialize_misc_info(SerializeClosure* soc) {
