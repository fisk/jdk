...
void HeapShared::initialize_roots() {
  if (is_loading_streaming_mode()) {
    StreamingArchiveHeapLoader::initialize_roots();
  }
}
...
oop HeapShared::orig_to_scratch_object(oop orig_obj) {
  MutexLocker ml(ArchivedObjectTables_lock, Mutex::_no_safepoint_check_flag);
  if (_orig_to_scratch_object_table != nullptr) {
    oop native_obj = nullptr;
    NativeAccess<>::oop_store(&native_obj, orig_obj);
    OopHandle orig(&native_obj);
...
int HeapShared::get_archived_object_permanent_index(oop obj) {
  ...
  if (CDSConfig::is_dumping_cached_code() && StoreCachedCode && _permanent_index_table == nullptr && !is_loading_streaming_mode()) {
    MappingArchiveHeapLoader::populate_permanent_object_table();
  }

  if (_permanent_index_table != nullptr) {
    if (_orig_to_scratch_object_table != nullptr) {
      oop native_obj = nullptr;
      NativeAccess<>::oop_store(&native_obj, obj);
      OopHandle orig(&native_obj);
      OopHandle* v = _orig_to_scratch_object_table->get(orig);
      if (v != nullptr) {
        obj = v->resolve();
...
oop HeapShared::get_archived_object(int permanent_index) {
  if (is_archived_heap_in_use()) {
    if (is_loading_streaming_mode()) {
      return StreamingArchiveHeapLoader::get_archived_object(permanent_index);
    } else {
      return MappingArchiveHeapLoader::get_archived_object(permanent_index);
    }
  }
  ...
  if (is_loading_streaming_mode()) {
    ...
    return MappingArchiveHeapWriter::get_perm_object_by_index(permanent_index);
  }
}
...
void HeapShared::mark_native_pointers(oop orig_obj) {
  ...
  if (java_lang_Class::is_instance(orig_obj)) {
    ...
    MappingArchiveHeapWriter::mark_native_pointer(orig_obj, java_lang_invoke_ResolvedMethodName::vmtarget_offset());
  }
}
...
void HeapShared::serialize_misc_info(SerializeClosure* soc) {
  if (!is_writing_streaming_mode() && soc->writing()) {
    MappingArchiveHeapLoader::serialize_misc_info(soc);
  }

  if (!is_loading_streaming_mode() && soc->reading()) {
    MappingArchiveHeapLoader::serialize_misc_info(soc);
  }
}
