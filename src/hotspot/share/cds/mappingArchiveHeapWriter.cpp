...
GrowableArrayCHeap<oop, mtClassShared>* MappingArchiveHeapWriter::_perm_objs = nullptr;
...
size_t MappingArchiveHeapWriter::create_objarray_in_buffer(GrowableArrayCHeap<oop, mtClassShared>* input,
...
int MappingArchiveHeapWriter::copy_source_objs_to_buffer(GrowableArrayCHeap<oop, mtClassShared>* roots,
                                                         GrowableArray<size_t>* permobj_seg_offsets) {
  ...
  MappingArchiveHeapLoader::set_permobj_segments(permobj_segments);
  int n = _perm_objs->length();
  return n;
}

oop MappingArchiveHeapWriter::get_perm_object_by_index(int permanent_index) {
...
template <typename T> void MappingArchiveHeapWriter::relocate_root_at(oop requested_roots, address buffered_roots_addr, int index, CHeapBitMap* oopmap) {
...
template <typename T> void MappingArchiveHeapWriter::add_permobj_segments_to_roots(GrowableArrayCHeap<oop, mtClassShared>* roots,
                                                                                   ArchiveHeapInfo* heap_info,
                                                                                   GrowableArray<size_t>* permobj_seg_offsets) {
  for (int i = 0; i <  permobj_seg_offsets->length(); i++) {
    size_t permobj_seg_bottom_offset = permobj_seg_offsets->at(i);
...
int MappingArchiveHeapWriter::get_permobj_segment_at(address buffered_addr, size_t* byte_size, int* permobj_segment_length) {
...
oop MappingArchiveHeapWriter::get_permobj_source_addr(int permobj_segment, int index) {
