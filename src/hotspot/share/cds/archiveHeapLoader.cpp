...
static oop interned_string(oopDesc* archive_object, OopStorage* oop_storage, JavaThread* thread) {
  ResourceMark rm(thread);
  int len;
  jchar* name = java_lang_String::as_unicode_string(archive_object, len, thread);

  oop heap_object = StringTable::intern((const char*)name, thread);
...
oop ArchiveHeapLoader::materialize_object(oopDesc* archive_object, Stack<CDSHeapTraversalEntry, mtClassShared>& dfs_stack, JavaThread* thread) {
  ...
  if (java_lang_String::is_instance(archive_object) && _oopmap.at(obj_bit + 1)) {
    // Interned string... link it to the string table
    return interned_string(archive_object, _oop_storage, thread);
  }

  return materialize_object_inner<COOPS, allow_gc>(archive_object, dfs_stack, thread);
}
