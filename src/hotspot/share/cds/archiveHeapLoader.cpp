...
GrowableArrayCHeap<oop*, mtClassShared>* ArchiveHeapLoader::_handles;
...
void ArchiveHeapLoader::initialize_roots() {
  ...
  _handles = new GrowableArrayCHeap<oop*, mtClassShared>();
  _roots = _oop_storage->allocate();
  _handles->append(_roots);
...
void ArchiveHeapLoader::materialize_objects() {
  ...
  for (int i = 0; i < _handles->length(); ++i) {
    _oop_storage->release(_handles->at(i));
  }
  delete _handles;
