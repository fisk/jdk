...
void StreamingArchiveHeapWriter::log_heap_region(ArchiveStreamedHeapInfo* heap_info) {
  ...
  for (int i = 0; i < _source_objs->length(); ++i) {
    ...
    address archive_object_start = current;
    size_t byte_size = source_oop->size() * BytesPerWord;
    address archive_object_end = archive_object_start + byte_size;

    intptr_t archive_object_offset = intptr_t(archive_object_start) - intptr_t(start);

    HeapShared::log_oop_info(&st, source_oop, archive_object_start, archive_object_end);

    current = archive_object_end;
  }
}
