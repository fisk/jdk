...
StreamingArchiveHeapWriter::BufferOffsetToSourceObjectTable*
  StreamingArchiveHeapWriter::_buffer_offset_to_source_obj_table = nullptr;
...
typedef ResourceHashtable<oop, size_t,
                          36137, // prime number
                          AnyObj::C_HEAP,
...
static int cmp_dfs_order(oop* o1, oop* o2) {
  size_t* o1_dfs = _dfs_order_table->get(*o1);
  size_t* o2_dfs = _dfs_order_table->get(*o2);
  if (o1_dfs != nullptr && o2_dfs == nullptr) {
    return -1;
  } else if (o1_dfs == nullptr && o2_dfs != nullptr) {
    return 1;
  } else if (o1_dfs == nullptr && o2_dfs == nullptr) {
    return 0;
  }
...
void StreamingArchiveHeapWriter::order_source_objs(GrowableArrayCHeap<oop, mtClassShared>* roots) {
  ...
  for (int i = 0; i < roots->length(); ++i) {
    ...
    while (!dfs_stack.is_empty()) {
      ...
      size_t* dfs_number = _dfs_order_table->get(obj);
      if (dfs_number != nullptr) {
        continue;
      }
      _dfs_order_table->put(obj, dfs_order++);
...
void StreamingArchiveHeapWriter::copy_roots_to_buffer(GrowableArrayCHeap<oop, mtClassShared>* roots) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (UseCompressedOops) {
      *arrayOop->obj_at_addr<uint32_t>(i) = (uint32_t)CompressedOops::encode(o);
    } else {
...
void StreamingArchiveHeapWriter::copy_source_objs_to_buffer(GrowableArrayCHeap<oop, mtClassShared>* roots) {
  for (int i = 0; i < _source_objs->length(); i++) {
    ...
    size_t* dfs_order = _dfs_order_table->get(src_obj);
    log_info(cds,heap)("src obj %ld", dfs_order == nullptr ? 0 : *dfs_order);
  }
...
inline void StreamingArchiveHeapWriter::store_oop_in_buffer(oop* buffered_addr, size_t buffer_offset) {
  *(size_t*)buffered_addr = buffer_offset;
}

inline void StreamingArchiveHeapWriter::store_oop_in_buffer(narrowOop* buffered_addr, size_t buffer_offset) {
...
template <typename T> void StreamingArchiveHeapWriter::relocate_root_at(oop obj, address buffered_roots, int index, CHeapBitMap* oopmap) {
  size_t offset = (size_t)((objArrayOopDesc*)buffered_roots)->obj_at_offset<T>(index);
  T* addr = (T*)(buffered_heap_roots_addr() + offset);
  if (obj != nullptr) {
    relocate_field_in_buffer<T>(obj, addr, oopmap);
  } else {
    store_oop_in_buffer(addr, _heap_roots_offset);
  }
}
...
void StreamingArchiveHeapWriter::relocate_embedded_oops(GrowableArrayCHeap<oop, mtClassShared>* roots,
  ...
  for (int i = 0; i < length; i++) {
    if (UseCompressedOops) {
      relocate_root_at<narrowOop>(roots->at(i), buffered_roots, i, heap_info->oopmap());
    } else {
      relocate_root_at<oop>(roots->at(i), buffered_roots, i, heap_info->oopmap());
    }
  }
