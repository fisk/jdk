...
void StreamingArchiveHeapWriter::copy_roots_to_buffer(GrowableArrayCHeap<oop, mtClassShared>* roots) {
  ...
  *mem = length;
  ...
  log_info(cds, heap)("archived obj roots[%d] = " SIZE_FORMAT " bytes, mem = %p", length, byte_size, mem);
...
static void mark_interned_string(size_t buffer_offset, BitMap* oopmap) {
  BitMap::idx_t obj_idx = bit_idx_for_buffer_offset(buffer_offset);
...
static bool needs_explicit_size(oop src_obj) {
  ...
  if (lh > Klass::_lh_neutral_value && Klass::layout_helper_needs_slow_path(lh)) {
    // Complicated instance - needs explicit size
    return true;
  }

  return lh == Klass::_lh_neutral_value;
}
...
void StreamingArchiveHeapWriter::print_oop(outputStream* st, oop source_oop, bool print_location) {
  if (source_oop == nullptr) {
    ...
    int dfs_index = *maybe_dfs_index;
    size_t buffer_offset = _dfs_to_archive_object_table[dfs_index];

    if (print_location) {
      st->print(PTR_FORMAT " ", buffer_offset);
    }

    st->print("(%d) ", dfs_index);
...
void StreamingArchiveHeapWriter::log_heap_region(ArchiveHeapInfo* heap_info) {
  ...
  for (int i = 0; i < _source_objs->length(); ++i) {
    size_t byte_size;
    oop source_oop = _source_objs->at(i);
    address archive_object = current;
    st.print(PTR_FORMAT ": @@ Object ", p2i(archive_object));

    print_oop(&st, source_oop, false /* print_location */);
    byte_size = source_oop->size() * BytesPerWord;

    address object_end = start + byte_size;
    current = object_end;

    os::print_hex_dump(&lsh, archive_object, object_end, UseCompressedOops ? 4 : 8, 32, 0);
