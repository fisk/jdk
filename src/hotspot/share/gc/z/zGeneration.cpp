...
uint ZGenerationYoung::compute_tenuring_threshold(ZRelocationSetSelectorStats stats) {
  ...
  uint upper_bound = MIN2(last_populated_age + 1u, (uint)MaxTenuringThreshold);
  uint lower_bound = MIN2(1u, upper_bound);
  uint tenuring_threshold = clamp((uint)round(tenuring_threshold_raw), lower_bound, upper_bound);
