...
#endif
  if (!cause_return) {
    Label no_rex_prefix, no_rex_b_prefix, not_ral;
    ...
    __ cmpb(Address(rbx, 0), NativeTstRegMem::instruction_rex_prefix);
    __ jcc(Assembler::notEqual, no_rex_prefix);
    __ addptr(rbx, 1);
    __ bind(no_rex_prefix);
    __ cmpb(Address(rbx, 0), NativeTstRegMem::instruction_rex_b_prefix);
    __ jcc(Assembler::notEqual, no_rex_b_prefix);
    __ addptr(rbx, 1);
    __ bind(no_rex_b_prefix);
    __ cmpb(Address(rbx, 0), 0xA8);
    __ jcc(Assembler::notEqual, not_ral);
    // Ral encoding is 1 byte smaller than the normal one; account for that
    __ subptr(rbx, 1);
    __ bind(not_ral);
    // Adjust return pc forward to step over the safepoint poll instructions
    __ addptr(rbx, 9);
    __ movptr(Address(rbp, wordSize), rbx);
  }
